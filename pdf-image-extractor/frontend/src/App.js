...
function App() {
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/extract-first-page', data, {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const normalizedResponse = await axios.post('http://localhost:5000/api/normalize-filename', {
        ...
        <div className="lg:w-2/3">
          {preview ? (
            ...
            <div className="bg-white rounded-xl shadow-md p-6 flex flex-col items-center justify-center h-[500px] text-gray-500">
              ...
              <p className="text-lg">Upload a PDF to see the preview here hahahaha</p>
            </div>
          )}
        </div>
        {/* Right side - Form */}
